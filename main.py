...
'''
    * 데이터 셋을 열어 보면 특성(컬럼) 일곱 개로 구성됨
        ...
        5) capacity : 수화물 용량
        6) safety : 안정성
        7) output : 차 상태 (unacc- 허용 불가수준, acc- 허용 가능수준, good- 양호, vgood- 매우 좋은)
...
# categorical_crossentropy 의 출력 값은 원-핫 인코딩된 결과가 출력됨
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])

'''
    모델은 완전연결층은 총 3개로 구성되어 있음.
'''
model.summary()
...
# 모델 평가
# 테스트 정확도 (test accuracy)의 결과값이 0.9768으로, 정확도는 약 97%
